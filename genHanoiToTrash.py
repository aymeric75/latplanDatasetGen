...
def return_moving_disc(state1, state2):
    ...
    #         moving_discs.append(i)
    # if len(moving_discs) > 1:
    #     return False
    # else:
    #     return moving_discs[0]


#print(return_moving_disc([1,1,2,3], [0,1,2,3]))
...
#print(return_pre_and_suc_tour([0,1,2,3], [0,1,2,0]))




# True if ele2 is smaller than ele1
# IF ele1 is a list (i.e. a EMPTY TOWER) then return TRUE
...
def is_valid_move(state1, state2):
    ...
    # print()
    # print("state1: {}, state2: {}".format(str(state1), str(state2)))
    # print("disc: {}".format(str(disc)))
    ...
    # print("to: {}, state2: {}".format(str(to), str(state2)))
    # print((not clear(to, state2)))

    if pre and effect:
        return True, "move_"+str(disc)+"_from_"+str(from_tour) + "_to_" + str(to_tour)
...
all_hanoi_states = generate_all_possible_states_hanoi(4)

print("jjjj")
print(len(all_hanoi_states))
...
for s1 in all_hanoi_states:
    for s2 in all_hanoi_states:
        ...
        if s1 != s2:
            ...
            valid, name = is_valid_move(s1, s2)
            if valid:
                all_hanoi_transitions_pddl_constraints.append([s1, s2])
...
###### FORMER CODE FOR ACTION LABEL CREATION
...
def return_hanoi_transitions_and_actions(args, parameters, version="simple"):
    ...
    elif version == "with_pddl_constraints":
        all_actions = all_actions_pddl_constraints
        flattened = [np.array(sub_array).flatten() for sub_array in all_hanoi_transitions_pddl_constraints]
        all_combis = flattened
